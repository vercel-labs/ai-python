...
MODEL_INPUT_UNSET: Any = _ModelInputUnset()
...
def _exclude_if_not_special(v: object) -> bool:
    return v != "special"
...
ModelInputKind = Literal["json", "special"]
...
class ToolResultPart(pydantic.BaseModel):
    ...
    # ``get_model_input``.  ``model_input_kind`` tags its shape
    # independently from ``result_kind``.  It's populated
    ...
    model_input: Any = pydantic.Field(
        default_factory=lambda: MODEL_INPUT_UNSET,
        exclude_if=cast(
            "Callable[[Any], bool]",
    ...
    model_input_kind: ModelInputKind = pydantic.Field(
        default="json",
        exclude_if=cast(
            "Callable[[Any], bool]",
            weakref.proxy(_exclude_if_not_special),
        ),
    )
    ...
    @classmethod
    def _restore_special_results(
        cls, data: Any, info: pydantic.ValidationInfo
    ) -> Any:
        """Rebuild typed special values after a JSON round-trip.

        ``result`` and ``model_input`` are ``Any``, so pydantic restores a
        serialized ``ContentOutput`` / ``MessageBundle`` as a plain dict.
        Their independent kind tags identify values that need coercion.
        """
        if not isinstance(data, dict):
            return data

        data = {**data}
        model_input = data.get("model_input", MODEL_INPUT_UNSET)
        if "model_input_kind" not in data and isinstance(
            model_input, SpecialToolResult
        ):
            data["model_input_kind"] = "special"
        if data.get("result_kind") == "special" and isinstance(
            data.get("result"), dict
        ):
            data["result"] = _SPECIAL_TOOL_RESULT_ADAPTER.validate_python(
                data["result"], context=info.context
            )
        if data.get("model_input_kind") == "special" and isinstance(
            data.get("model_input"), dict
        ):
            data["model_input"] = _SPECIAL_TOOL_RESULT_ADAPTER.validate_python(
                data["model_input"], context=info.context
            )
        return data
    ...
    def set_model_input(self, value: Any) -> None:
        ...
        object.__setattr__(self, "model_input_kind", self.kind_for(value))
