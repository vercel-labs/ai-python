...
Owns everything the v3 spec defines: the prompt encoding (data-URL file
parts, ``image-data``/``file-data`` tool-result content), the ``reasoning``
request field (an ``{effort, summary}`` object for providers without a
dedicated branch), and the stream-part vocabulary.  Version-stable pieces
come from :mod:`._shared`.
"""
...
# ---------------------------------------------------------------------------
# Message list → v3 prompt
# ---------------------------------------------------------------------------


def _file_part_to_inline(part: types.messages.FilePart) -> dict[str, Any]:
...
async def _file_part_to_wire(part: types.messages.FilePart) -> dict[str, Any]:
    """Convert a :class:`FilePart` to a v3 ``file`` content part.

    v3 has no URL passthrough for prompt files: downloadable URLs are
    fetched client-side and inlined as data URLs.
    """
    data = part.data
    if isinstance(data, str) and types.media.is_downloadable_url(data):
        downloaded, _ = await models.core.helpers.files.download(data)
        data = downloaded
    ...
    if isinstance(value, types.messages.ContentOutput):
        ...
        for item in value.value:
            if isinstance(item, types.messages.FilePart):
                parts.append(_file_part_to_inline(item))
            else:
            ...
            case "user":
                ...
                for p in msg.parts:
                    if isinstance(p, types.messages.TextPart):
                        ...
                        content.append(await _file_part_to_wire(p))
...
def _apply_reasoning(
    ...
    *,
    ...
    body["reasoning"] = {
        ...
        if not isinstance(value, models.ModelProviderDefault)
    }


# ---------------------------------------------------------------------------
# v3 stream parts → public Event
# ---------------------------------------------------------------------------
