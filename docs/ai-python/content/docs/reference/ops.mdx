...
- `questions`: A nonempty Pydantic model whose fields contain
  `ChoiceQuestion`, `ScoreQuestion`, or `BooleanQuestion` values.
- `output_type`: A Pydantic model class with the same fields as `questions`.
  Each field must use the answer type corresponding to its question.
- `params`: Optional `EvaluationParams`.

Returns `Item[Answers]`. The item value is an instance of `output_type`.
