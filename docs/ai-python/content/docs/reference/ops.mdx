...
- `questions`: Nonempty mapping of question IDs to `ChoiceQuestion`,
  `ScoreQuestion`, or `BooleanQuestion` values.
- `params`: Optional `EvaluationParams`.

Returns `Item[Evaluation]`.

`Evaluation` fields:

- `answers`: Every question ID mapped to its typed answer.
- `rounding`: Optional provider-declared `probability_decimals` and
  `score_decimals`. Each is an integer from `0` to `15`, or `None`.
