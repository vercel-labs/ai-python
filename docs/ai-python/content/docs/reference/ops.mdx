...
- `state`: Shared JSON-compatible string, object, or array, or a Pydantic model
  (serialized in JSON mode with field aliases).
- `questions`: A nonempty `BaseQuestionModel` subclass instance or mapping whose
  fields contain `ChoiceQuestion`, `ScoreQuestion`, or `BooleanQuestion` values.
  The typed model specializes `BaseQuestionModel[Answers]`, where `Answers`
  subclasses `BaseAnswerModel` with matching field names and corresponding
  answer types. Ordinary Pydantic models are not accepted for typed questions.
- `params`: Optional `EvaluationParams`.

For typed questions, the return type is inferred from the declared answer
model:

```python
class Answers(ai.ops.BaseAnswerModel):
    approved: ai.ops.BooleanAnswer

class Questions(ai.ops.BaseQuestionModel[Answers]):
    approved: ai.ops.BooleanQuestion

result = await ai.ops.experimental_evaluate(model, state, Questions(
    approved=ai.ops.BooleanQuestion(instructions="Is it approved?"),
))
# result.value is an Answers instance
```

Mapped questions return
