...
- `Span.set_attributes(attributes=None, /, **kwargs)` merges attributes into a user
  span:
  ...
  `set_attributes()` only works on spans created with a string name (their data is
...
1. subclassing `Adapter`, overriding its `wrap_span` method (not to be confused with the
   decorator), and, optionally, storing state on the class, and so on;
...
3. implementing `AdapterProtocol` from scratch;
...
Setup takes two steps: configure OpenTelemetry export, then register the
adapter.
...
`configure(*, endpoint=None)` performs the OpenTelemetry configuration
ceremony: it creates a `TracerProvider` with a batching OTLP/HTTP exporter,
sets it as the global tracer provider, and returns it. It is equivalent to:
...
Without an `endpoint` argument, the exporter reads the standard environment
variable, so you can pick the backend without code changes:
...
If your application already performs the ceremony itself, skip `configure()`
and only register the adapter: `OtelAdapter` uses the global tracer provider
unless you pass one.
...
Regular agent tracing only requires you to configure the export and register
an adapter: 
...
- Adapters: `Adapter`, `AdapterProtocol`, `wrap_span`, `register`,
  `unregister`, `is_enabled`.
- Sinks and clocks: `Sink`, `use_sink`, `DictSink`, `push_all`, `use_time`,
  `now_ns`.

`ai.experimental_telemetry.otel` exports `configure` and `OtelAdapter`.
