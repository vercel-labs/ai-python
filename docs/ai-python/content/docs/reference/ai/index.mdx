...
- `resolve(tool_call)`: Convert model tool call parts into executable
  `BoundToolCall` objects.
- `add(message)`: Append messages to history.

### ToolCall

`BoundToolCall` is the executable runtime object produced by `Agent.resolve`.
`Context.resolve` remains available for backward compatibility.
