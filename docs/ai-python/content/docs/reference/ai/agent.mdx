...
Use the class methods below when code called by an agent needs information
about its active run or tool call:
...
async def inspect_context() -> str:
    agent = MyAgent.current_agent()
    tool_call = MyAgent.current_tool_call()
    return f"{type(agent).__name__}: {tool_call.id}"
```

### current_agent

`Agent.current_agent()` returns the agent whose stream is executing in the
current context. Calling it outside an agent stream raises `LookupError`.
Its return type is `Self`, so calling `MyAgent.current_agent()` returns
`MyAgent` to type checkers.

### current_tool_call

`Agent.current_tool_call()` returns the `ToolCall` executing in the current
