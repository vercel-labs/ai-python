...
events, and generated media to and from the AI Gateway v4 API.
...
`GatewayV3Protocol` speaks the previous AI Gateway v3 API. Pass it as a
protocol override to keep using the v3 wire protocol; requests are routed to
the matching `/v3/ai` endpoints automatically.
