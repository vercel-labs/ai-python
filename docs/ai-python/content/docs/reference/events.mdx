...
- `Retry`: Discards everything since the most recent `StreamStart`: the
  in-progress assistant message and any tool results for it. The
  library does not emit this yet; it is for custom loops that retry a model
  call after it already produced events.

Agent event union:

- `AgentEvent`: everything an agent run can yield: `Event` plus
  `ToolCallResult`, `HookEvent`, `PartialToolCallResult`, `RunBlocked`,
  and `Retry`.
