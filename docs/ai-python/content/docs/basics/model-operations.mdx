...
description: Generate media and embeddings, transcribe audio, rerank documents, and evaluate state.
type: guide
summary: Use dedicated models for images, video, speech, embeddings, transcription, reranking, and evaluation.
---
...
Use `experimental_evaluate` to ask choice, score, and boolean questions about one shared state. Define matching question and answer models; the question model's generic argument specifies its answer model:
...
print(result.value.department.choice)
```

Pydantic state is serialized to JSON before it is sent to the provider. Strings,
JSON objects, and JSON arrays are also accepted. When question IDs are dynamic,
pass a mapping instead of a `BaseQuestionModel`. The item value is then a
validated answer mapping:
...
Both forms return an `Item`, so usage, warnings, framework metadata, and
provider metadata remain available. This operation specifically supports
...
parameter; it is empty when there are none. Framework-normalized operation data
is available on `metadata`, while provider-specific response data is available
on `provider_metadata`.
