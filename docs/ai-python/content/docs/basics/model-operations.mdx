...
For image-to-video models, pass a `VideoPrompt` with the starting image:
...
`VideoPrompt` also accepts role-tagged first/last-frame images through
`frame_images` and reference media through `references`.
