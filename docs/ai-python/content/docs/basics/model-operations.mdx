...
The result is an `Item[Answers]`, so usage, warnings, framework metadata, and
provider metadata remain available on the item. This operation specifically
supports [Jev](https://vercel.com/ai-gateway/models/jev).
