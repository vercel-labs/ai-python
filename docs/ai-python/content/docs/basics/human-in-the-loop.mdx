...
servers. For that reason, the SDK providers an alternative way for handling
hook resolutions.

Using the serverless flow only requires you to update the `agent.run` callsite
(e.g. your serverless endpoint code).

> provide example of abort_pending_hook

When handling the hook event, call `abort_pending_hook` to interrupt the loop
