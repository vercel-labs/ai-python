...
## Replay an existing assistant turn

When the last message has `replay=True`, `ai.stream` does not call the
provider. It emits replay events from the existing assistant message so resume
flows can dispatch the same tool calls again.
