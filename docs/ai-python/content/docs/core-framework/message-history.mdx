...
Replay flags are serialized when true and omitted when false.
...
objects, providers, and hooks on the next request. Resume state such as replay
markers, cached tool results, and model-facing tool input is included when
present.
