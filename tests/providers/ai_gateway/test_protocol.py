...
- ``generate_image``: the image-model endpoint, exercised end-to-end via
  ``ops.generate_image()`` with a provider wired to an
  ``httpx.MockTransport``
- ``generate_video``: the video-model endpoint (SSE), exercised the same
  way via ``ops.generate_video()``
- ``generate_audio``: the speech-model endpoint, exercised the same way
  via ``ops.generate_audio()``
...
async def test_build_request_body_with_output_type() -> None:
...
def test_tool_result_output_text() -> None:
    result = protocol._tool_result_output(_tool_result_part("hi"))
    assert result == {"type": "text", "value": "hi"}


def test_tool_result_output_json() -> None:
    result = protocol._tool_result_output(_tool_result_part({"key": "value"}))
...
async def test_image_429_rate_limit_error() -> None:
    def handler(req: httpx.Request) -> httpx.Response:
        return httpx.Response(
            429,
            json={
...
async def test_audio_429_rate_limit_error() -> None:
    def handler(req: httpx.Request) -> httpx.Response:
        return httpx.Response(
            429,
            json={
